...
classDiagram
    ...
    %% [Controller] 시스템 경계 제어 영역 (JSP/서블릿 바운더리 매핑)
    class 캐릭터생성_컨트롤러 {
        <<controller>>
    ...
    class 전사 {
        +전사(플레이어ID: String, 캐릭터명: String, 레벨: int)
        +스킬발동() String
        +데미지계산() int
    }

    class 마법사 {
        +마법사(플레이어ID: String, 캐릭터명: String, 레벨: int)
        +스킬발동() String
        +데미지계산() int
    }
    ...
    캐릭터생성_컨트롤러 ..> Create_Character_UI : 3. 결과 Forward 전달
